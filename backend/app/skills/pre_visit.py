from app.services.demo_data import crm_interaction_source
...
def run_pre_visit_skill(query: str, customer_id: str = "liu-min") -> dict:
    ...
    evidence = retrieve_approved_evidence(query, customer_search_context(customer_context))
    institution = read_institution_status(customer_id) if any(word in query for word in ("机构", "进药", "进院", "院内")) else None
    return {
        "skill_id": SKILL_ID,
